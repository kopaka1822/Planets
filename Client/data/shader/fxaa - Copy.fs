...
vec3 FxaaPixelShader()
{
	...
	*/
	...
	vec3 rgbNW = texture2D(bg, posPos.st - (s * (0.5 + FXAA_SUBPIX_SHIFT))).rgb;
	vec3 rgbNE = texture2D(bg, posPos.st + (vec2(s.x, -s.y) * (0.5 + FXAA_SUBPIX_SHIFT))).rgb;
	vec3 rgbSW = texture2D(bg, posPos.st + (vec2(-s.x, s.y) * (0.5 + FXAA_SUBPIX_SHIFT))).rgb;
	vec3 rgbSE = texture2D(bg, posPos.st + (s * (0.5 + FXAA_SUBPIX_SHIFT))).rgb;
